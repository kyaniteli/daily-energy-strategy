...
class FusionStrategy:
    ...
    def analyze_chenye(self):
        ...
        df = df[
            ...
            (df['price'] > 2.5) # 2.5元以下有退市风险，不碰
        ]
        
        print(f"   初筛剩余: {len(df)} 只，开始计算历史位置...")
        
        # 为了防止GitHub超时，只取前100个看起来像样的（比如量比活跃一点的，或者随机取，这里简单按市值排序取小盘）
        # 也可以全跑，但建议限制一下
